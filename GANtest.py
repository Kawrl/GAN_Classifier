...
def gantest(model_dir):
    
    #args = load_parser()
    test_dir = model_dir / 'imgs'
    ...
    hard_case_plot_name =test_dir_str + '_GANtest_hard_cases.png'
    prediction_plot_name = test_dir_str + '_GANtest_predictions.png'
    ...
    test_data = CropsData(test_dir,real_set_transform)

    model_msg = f'Evaluating pretrained resnet on model: {model_name}'
    print(model_msg)
    logging.info(model_msg)

    data_info = f'There are {len(test_data)} samples in test set.'
    print(data_info)
    logging.info(data_info)

    plot_samples(dataset=test_data,
                mean=0.5,
                std=0.5,
                suptitle='Test set samples',
    ...
    # Load the model:
    # Load trained model:
    resnet.load_state_dict(torch.load('./resnet_real_classifier.pth'))
